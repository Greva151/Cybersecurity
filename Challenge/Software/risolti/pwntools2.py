...
def main():
    ...
    for i in range(100):
        ...
        if stringa[8][:-1] == "64-bit":
            if stringa[6] == "packed":
                r.send(p64(int(stringa[5][2:],16)))
            elif stringa[6] == "unpacked":
                r.send(u64(stringa[5].encode()))

        elif stringa[8][:-1] == "32-bit":
            if stringa[6] == "packed":
                r.send(p32(int(stringa[5][2:],16)))
            elif stringa[6] == "unpacked":
                r.send(u32(stringa[5].encode()))
    r.interactive()
    r.close()
